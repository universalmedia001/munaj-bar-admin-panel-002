...
type ModalStep = 'backup_prompt' | 'reset_confirmation';
...
export const ClearBusinessDataModal: React.FC<ClearBusinessDataModalProps> = ({
  ...
  const [step, setStep] = useState<ModalStep>('backup_prompt');
  ...
  const handleBackupAndContinue = async () => {
    ...
    setErrorMsg(null);
    try {
      const result = await createBackup('BEFORE_RESET');
      if (result.success) {
        setBackupNotice('✓ Backup created successfully. You can now proceed with the reset.');
        setStep('reset_confirmation');
      } else {
        // Backup failed — STOP, do not proceed to reset
        setErrorMsg(result.error || 'Backup failed. Reset has been stopped to protect your data.');
      }
    } catch (err: any) {
      setErrorMsg(err?.message || 'Backup failed. Reset has been stopped to protect your data.');
    } finally {
  ...
  const handleClose = () => {
    if (isProcessing || isBackingUp) return;
    onClose();
  };

  // Reset step state when modal is reopened
  const handleModalClose = () => {
    if (isProcessing || isBackingUp) return;
    // Reset to initial state for next open
    setStep('backup_prompt');
    setConfirmText('');
    setErrorMsg(null);
      ...
      <div className="w-full max-w-lg bg-zinc-950 border border-red-900/60 rounded-2xl shadow-2xl overflow-hidden text-zinc-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-5">
          ...
          {step === 'backup_prompt' && (
            <>
              {/* Backup prompt */}
              <div className="p-4 rounded-xl bg-blue-950/30 border border-blue-900/50 text-xs space-y-2">
                <div className="flex items-center gap-2 text-blue-300 font-semibold text-sm">
                  <Archive className="w-4 h-4" />
                  <span>Would you like to back up your current business data before resetting?</span>
                </div>
                <p className="text-blue-300/80 leading-relaxed">
                  A backup stores your current sales, products, shifts, and expenses in Supabase so you can restore them later. This is strongly recommended before resetting.
                </p>
              </div>
              ...
              <div className="space-y-2.5">
                <button
                  ...
                  className="w-full p-3.5 rounded-xl bg-blue-600/90 hover:bg-blue-600 text-white text-xs font-bold transition-all flex items-center gap-2.5 disabled:opacity-60"
                >
                  {isBackingUp ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>Creating backup... Please wait.</span>
                    </>
                  ) : (
                    <>
                      <Database className="w-4 h-4" />
                      <span>Backup &amp; Continue Reset</span>
                    </>
                  )}
                </button>
                ...
                <button
                  ...
                  disabled={isBackingUp}
                  className="w-full p-3.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold transition-all flex items-center gap-2.5 border border-zinc-700"
                >
                  <RefreshCw className="w-4 h-4 text-amber-400" />
                  <span>Continue Without Backup</span>
                </button>
                ...
                <button
                  ...
                  disabled={isBackingUp}
                  className="w-full p-3.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-400 text-xs font-semibold transition-all flex items-center gap-2.5 border border-zinc-800"
                >
                  <X className="w-4 h-4" />
                  <span>Cancel</span>
                </button>
